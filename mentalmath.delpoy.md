...
The `.env` directory should appear. Activate the environment:
...
Our django app is basically ready, but using the built-in django server on production is a bad idea. So we will configure the following setup:
...
server {
    ...
    location / {
		...
		# Set the proper path for the a unix-socket file.
...
Open ports 80 and 443 in your iptables, and do not forget to remove the rule about port 42673 (we opened it earlier for testing):
...
# directory on the django app (where manage.py is located)
...
I reccomend you to visit [this page](https://docs.djangoproject.com/en/5.1/howto/deployment/checklist/) and doublecheck everything. 
